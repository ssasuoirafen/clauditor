...
| Plugin marketplaces | https://code.claude.com/docs/en/plugin-marketplaces | [official] | Marketplace = `marketplace.json` catalog in a git repo. Official marketplaces: `claude-plugins-official` (Anthropic-curated), `claude-plugins-community` (third-party submissions). Submit via claude.ai or Console forms. `claude plugin validate` runs the same checks as the review pipeline. Approved plugins pinned to commit SHA in the community catalog; CI bumps pin on new commits. | 2026-06-20 |
...
| Agent Skills | `checks/entities.md` E01-E07 (skill body/description/size), E06 (>300 lines flag); `decision-matrix.md` (multi-step auto-trigger -> skill) |
...
| Plugins | `checks/entities.md` 5j E30-E34 (plugin vs local collision, intent overlap); `checks/recon.md` (enabledPlugins inventory) |
| Plugin marketplaces | `checks/entities.md` E34 (correct plugin for project type not enabled); `checks/security-config.md` F06 (unused server) |
...
3. **Skill body size** - the live docs do NOT state a 300-line body limit. The rubric's E06
   (`>300 lines -> flag`) is a conservative heuristic, not a spec requirement. The rubric correctly
   phrases it as a recommendation. No spec violation - but the currency agent should note this is
   internally derived.
...
5. **`allowed-tools` in a skill is pre-approval, not restriction** - confirmed live: the docs
   state the skill retains access to all session tools regardless of `allowed-tools`. The rubric's
   entities.md correctly notes this. Agents (not skills) provide real tool restriction.
...
7. **Five hook handler types confirmed** - command, http, mcp_tool, prompt, agent. The security-
   config.md M15 table is correct.
...
9. **Plugin `settings.json` only honors `agent` and `subagentStatusLine` keys** - confirmed live.
   The rubric does not currently audit plugin settings.json content; this is a potential gap for
   the currency agent to evaluate.
